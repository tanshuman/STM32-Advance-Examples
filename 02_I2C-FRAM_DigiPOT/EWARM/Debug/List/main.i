...
int main()
{
  ...
  float read_data[2];
  USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800)), 'A');
  
  while(1)
  {
    if(I2C1_WriteBuffer((uint8_t*)&write_data, ((uint8_t)0xAC), 2, 0x0F,2 * sizeof(float)) == 0)
      USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800)), 'A');
    
    if(I2C1_ReadBuffer((uint8_t*)&read_data, ((uint8_t)0xAC), 2, 0x0F,2 * sizeof(float)) == 0)
      USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800)), 'B');
    
    
      if(write_data[0] == read_data[0] && write_data[1] == read_data[1]) ; 
        
      else
        USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800)), 'C');
...
int readADC1()
{
  ...
  while(ADC_GetFlagStatus(((ADC_TypeDef *) (((uint32_t)0x40000000) + 0x00012400)), ((uint32_t)0x00000004)) == RESET) ;
  
  ADC_StopOfConversion(((ADC_TypeDef *) (((uint32_t)0x40000000) + 0x00012400)));
  
  return ADC_GetConversionValue(((ADC_TypeDef *) (((uint32_t)0x40000000) + 0x00012400)));
}

void ext_digipot_test()
{
  if(I2C1_WriteBuffer((uint8_t*)&digi_1, ((uint8_t)0x5C), 1, 0x00, sizeof(digi_1)) == 0)
   USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x00013800)), 'K');
